...
// Adaptado para ECS Fargate: todas las llamadas API van via nginx (puerto 80)
// nginx actua como API Gateway y rutea por metodo HTTP a cada microservicio

const API_URL = '/api/products'; // URL relativa - nginx rutea internamente
...
function renderizarTabla(productos) {
    ...
    productos.forEach(p => {
        ...
        tr.innerHTML = `
            ...
            <td>${p.categoria || '-'}</td>
            <td>
                <button class="btn-editar" onclick="editarProducto(${p.id},'${escapar(p.nombre)}','${escapar(p.descripcion||'')}',${p.precio},${p.stock},'${escapar(p.categoria||'')}')">Editar</button>
